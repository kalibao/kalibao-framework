  ...
  $.kalibao.crud.Edit.prototype.validate = function(validators, $container) {
    ...
    for (var attributeName in validators) {
      ...
      for (var i in validators[attributeName].script) {
        var messages = [];
        var attribute = {input:'div[data-validator-id=' + id + '], input[data-validator-id=' + id + '], textarea[data-validator-id=' + id + '], select[data-validator-id=' + id + ']'};
        ...
        $inputs.each(function () {
          ...
          if ($input.is('div')) {
            ...
            attrValue = $input.find('input[name="' + attrName + '"]:checked').val();
          } else {
            attrValue = $input.val();
          }
          var $formGroup = $input.closest('.form-group');
          var $helpInline = $formGroup.find('.help-inline');

          try {
            validators[attributeName].script[i](attribute, attrValue, messages);
          } catch(e) {
          }
